...
import streamlit as st  # Добавь, если не импортировал
...
def extract_vacancy_data(html):
    try:
        ...
        raise RuntimeError(f"Ошибка при парсинге вакансии: {e}")
...
def extract_resume_data(html):
    try:
        ...
        raise RuntimeError(f"Ошибка при парсинге резюме: {e}")
